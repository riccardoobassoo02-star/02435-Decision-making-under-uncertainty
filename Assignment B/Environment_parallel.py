...
def run_environment(policy, n_experiments=1, plot=False):  
    ...
    for day in range(0, n_experiments): 
        ...
        # Save logs
        all_logs.append({"day": day, "log": day_log})
...
if __name__ == "__main__": # executes the following block only if this file is run directly (not imported as a module)
    ...
    # Save mean and std of each experiment
    means = np.mean(results, axis = 1)
    stds  = np.std(results, axis = 1)

    final_results = np.column_stack((means, stds))

    # guardar en CSV
    np.savetxt(
        f"SP_policy_log_{n_experiments}_exp_{n_repetitions}_reps.csv",
        final_results,
        delimiter=",",
        header="mean,std",
        comments="",
