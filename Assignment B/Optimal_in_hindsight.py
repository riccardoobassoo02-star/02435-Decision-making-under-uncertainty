...
price_data = pd.read_csv(DATA_DIR / 'v2_PriceData.csv') 
...
T0_prev = data['T1'] # every day's initial temperature
T0          = data['T1'] # initial indoor temperature (°C)
H0          = data['H'] # initial indoor humidity (%)
...
def solve_milp(price,occ_r1,occ_r2):  
    ...
    for t in model.T: 
        ...
        # from the fourth hour of the day, we can start to consider the ventilation inertia 
        # remember: python index starts from 0
        if t >= 3:
    ...
    if result.solver.termination_condition != TerminationCondition.optimal:
        print(f"Warning: Day {day} did not solve to optimality") # check if an optimal solution couldn't be found
...
results_df.to_csv(OUTPUT_DIR / 'Results_optimal_in_hindsight.csv', index=False)
print("Results saved to Results_optimal_in_hindsight.csv")
