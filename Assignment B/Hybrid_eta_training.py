...
# FORWARD PASS
def forward_pass(eta, initial_state, n_trajectories=None):
    """
    Roll out N trajectories using the current hybrid policy.
    The resulting actions are then kept fixed in the backward pass.
    """
    ...
    states  = [[None] * T for _ in range(n_sim)]
    actions = [[None] * T for _ in range(n_sim)]
    costs   = [[0.0]  * T for _ in range(n_sim)]
...
# BACKWARD PASS
def backward_pass(states, actions, costs, eta_current, alpha=1.0, K=10):
    """
    Policy evaluation with fixed actions from the forward pass.
    ...
    for t in reversed(range(T)):
        features = np.zeros((n_traj, n_features))
        targets  = np.zeros(n_traj)
...
def compute_avg_fit_error(states, actions, costs, eta, K=10):
    ...
    return total_error / (T * n_traj)


# POLICY COST EVALUATION on independent rollouts
def evaluate_policy(eta, n_eval=50):
    """
    Simulate n_eval trajectories under the current hybrid policy and
    return the average cumulative cost. Used to track best_eta.
    Note: n_eval is kept small (50) due to the high cost of hybrid solves.
    """
    total_cost = 0.0
    for _ in range(n_eval):
        ...
        cum_cost = 0.0
        for t in range(T):
            action = solve_hybrid_for_training(state, eta, L_lookahead=4, B=3)
            action = apply_overrule(state, action)
...
# Warm start from existing ADP eta if available.
# The Task 4 weights provide a meaningful initial estimate since both
# value functions approximate the same cost-to-go over the same state space.
if Path("eta_weights.npy").exists():
    eta = np.load("eta_weights.npy")
    print("Loaded eta_weights.npy as warm start.")
else:
...
# Improves training diversity without increasing forward pass cost.
replay_capacity = 5 * N
replay_buffer   = []
...
validation_states, validation_actions, validation_costs = forward_pass(
    eta, initial_state, n_trajectories=N_val
)
...
for i in range(N_iterations):
    ...
    # 3. Sample training batch from replay buffer.
    ...
    train_states  = [replay_buffer[k][0] for k in idx]
    train_actions = [replay_buffer[k][1] for k in idx]
    train_costs   = [replay_buffer[k][2] for k in idx]

    # 4. Backward policy evaluation on sampled batch.
    eta_candidate = eta.copy()
    for j in range(J):
        eta_candidate = backward_pass(
            train_states,
    ...
    # Track best weights by policy cost (not fit error).
...
print(f"\nBest policy cost: {best_error:.4f}")
print("Saved:")
print("  eta_weights_hybrid.npy")
print("  eta_weights_hybrid_best.npy")
print("  eta_weights_hybrid.csv")