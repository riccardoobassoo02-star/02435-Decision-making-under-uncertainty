...
def generate_exogenous(state):
    ...
    # print(f"Generated exogenous sample in {time.time() - t:.2f} seconds.")
    return {
        "price_t":       price_new,
...
def forward_pass(eta, initial_state, n_trajectories=None, K_policy=5):
    """
    ...
    """
    n_sim = N if n_trajectories is None else n_trajectories
    states  = [[None] * L for _ in range(n_sim)]
    actions = [[None] * L for _ in range(n_sim)]
    costs   = [[0.0]  * L for _ in range(n_sim)]
    ...
    for n in range(n_sim):
        ...
        for t in range(L):
            ...
            action = solve_forward_pass_milp(state, eta, K_policy=K_policy)
            if action is None:
                raise RuntimeError(
...
def compute_avg_fit_error(states, actions, costs, eta, K=10):
    ...
    return total_error / (L * n_samples)
...
def evaluate_policy(eta, n_eval=200):
    """Simulate policy given by eta and return average cumulative cost."""
    total_cost = 0.0
    for i_eval in range(n_eval):
        ...
        for t in range(L):
            ...
            if t < L - 1:
                ...
                state = simulate_transition(state, action, exogenous)
        total_cost += cum_cost
    return total_cost / n_eval

# Create fixed validation set (deterministic seed, then restore RNG state)
_rng_state = np.random.get_state()
np.random.seed(0)
validation_states, validation_actions, validation_costs = forward_pass(eta, initial_state, n_trajectories=N_val)
np.random.set_state(_rng_state)

for i in range(N_iterations):
    print(f"Iteration {i+1}/{N_iterations}")

    # 1) Generate new trajectories with current policy
    states, actions, costs = forward_pass(eta, initial_state)

    # 2) Append individual trajectories to replay buffer
    for n in range(len(states)):
        replay_buffer.append((states[n], actions[n], costs[n]))
        # enforce capacity
        if len(replay_buffer) > replay_capacity:
            replay_buffer.pop(0)

    # 3) Build a training batch of exactly N trajectories sampled from replay buffer
    if len(replay_buffer) == 0:
        raise RuntimeError("Replay buffer is empty — no data to train on")

    if len(replay_buffer) >= N:
        idx = np.random.choice(len(replay_buffer), size=N, replace=False)
    else:
        idx = np.random.choice(len(replay_buffer), size=N, replace=True)

    train_states  = [[None] * L for _ in range(N)]
    train_actions = [[None] * L for _ in range(N)]
    train_costs   = [[0.0]  * L for _ in range(N)]

    for ii, buf_i in enumerate(idx):
        s,a,c = replay_buffer[buf_i]
        train_states[ii]  = s
        train_actions[ii] = a
        train_costs[ii]   = c

    # 4) Backward pass (policy evaluation) using the sampled training set
    eta_current = eta.copy()
    for j in range(J):
        eta_current = backward_pass(train_states, train_actions, train_costs, eta_current)

    # 5) Soft update
    beta = 0.2
    eta = (1 - beta) * eta + beta * eta_current

    # 6) Monitoring: training fit, validation fit, and policy evaluation cost
    ...
    policy_cost = evaluate_policy(eta, n_eval=200)

    print(f"  train fit error: {train_error:.4f} | val fit error: {val_error:.4f} | policy avg cost: {policy_cost:.4f}")

    # Track best policy by evaluation cost
    if policy_cost < best_error:
        best_error = policy_cost
        best_eta = eta.copy()

    if val_error < convergence_tol:
        print(f"Converged (val error) at iteration {i+1}")
        break

# SAVE RESULTS
np.save("eta_weights.npy", eta)
np.save("eta_weights_best.npy", best_eta)
